...
const DEFAULT_GLOBAL = ["#2563eb", "#f97316", "#334155", "#64748b", "#0f172a", "#7c3aed", "#000000", "#ea580c"]

function loadGlobal(): string[] {
  try { return JSON.parse(localStorage.getItem(STORAGE_KEY) || "null") ?? DEFAULT_GLOBAL } catch { return DEFAULT_GLOBAL }
}
...
function GradientCanvas({
  hue, sx, sy, onChange,
}: { hue: number; sx: number; sy: number; onChange: (s: number, v: number) => void }) {
...
function RangeSlider({
  value, onChange, bg, className = "",
}: { value: number; onChange: (v: number) => void; bg: string; className?: string }) {
  return (
    <div className={`relative h-3 rounded-full overflow-hidden ${className}`} style={{ background: bg }}>
      <input
      ...
      <div
        ...
        style={{ left: `calc(${value * 100}% - 6px)`, boxShadow: "0 0 0 1px rgba(0,0,0,0.25)" }}
      />
    </div>
  )
}
...
export function   ColorPicker({ color, onChange, className = "" }: ColorPickerProps) {
  ...
  const parseColor = (c: string): [number, number, number, number] => {
    ...
    try { const [r, g, b] = hexToRgb(c); return [r, g, b, 1] } catch { return [0, 0, 0, 1] }
  }
  ...
  const [hex, setHex] = useState(color.startsWith("#") ? color.replace("#", "") : rgbToHex(r0, g0, b0).replace("#", ""))
  ...
  useEffect(() => { setGlobalColors(loadGlobal()) }, [])
  ...
  const handleGradient = (s: number, v: number) => { setSat(s); setVal(v); emit(hue, s, v, alpha) }
  const handleHue = (v: number) => { const h = v * 360; setHue(h); emit(h, sat, val, alpha) }
  const handleAlpha = (a: number) => { setAlpha(a); emit(hue, sat, val, a) }
  ...
  const addToGlobal = () => {
    ...
    if (!globalColors.includes(c)) {
      ...
      setGlobalColors(next); saveGlobal(next)
    }
  }
  ...
  const previewColor = alpha < 1 ? `rgba(${r},${g},${b},${alpha.toFixed(2)})` : rgbToHex(r, g, b)
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-3 py-2 border-b border-slate-100">
          <span className="text-[13px] font-semibold text-slate-700">Color Picker</span>
        ...
        <div className="px-3 pt-3 pb-2 space-y-2">
          ...
          >
            Color Filters <ChevronDown className={`h-3 w-3 transition-transform ${showFilters ? "rotate-180" : ""}`} />
          </button>
          {showFilters && (
            <div className="grid grid-cols-5 gap-1">
              {["saturate", "brightness", "contrast", "grayscale", "invert"].map((f) => (
                <button
                  key={f}
                  title={f}
        ...
        <div className="px-3 py-2">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[12px] font-semibold text-slate-700">Global Colors</span>
                ...
                <div key={i} className="relative group">
                  <button
                    ...
                    onClick={() => {
                      ...
                      const [h, s, v] = rgbToHsv(r, g, b)
                      setHue(h); setSat(s); setVal(v); setHex(c.replace("#", ""))
                      emit(h, s, v, alpha)
                    }}
                  />
                  <button
            ...
            <div className="space-y-1 max-h-24 overflow-y-auto">
              {globalColors.map((c, i) => (
                <div key={i} className="flex items-center gap-2 group">
                  <div className="w-5 h-5 rounded border border-slate-200" style={{ backgroundColor: c }} />
                  <span className="text-[11px] text-slate-600 flex-1">{c}</span>
                  <button onClick={() => removeGlobal(i)} className="hidden group-hover:block text-red-400 hover:text-red-500">
                    <X className="h-3 w-3" />
                  </button>
                </div>
              ))}
