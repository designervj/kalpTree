...
// /**
//  * Parses a rawe CSS string (typically from :root or style tag) and returns a GlobalStyleModel.
//  */
...
import {
  BodyStyle,
  BrandColors as BrandColorsLegacy,
  ButtonBaseStyle,
  ButtonColors,
  HeadingStyle,
  HeadingKey,
...
// ──────────────────────────────────────────────────────────────
// Brand color model v2
// We renamed the old `dark` token to `background` because it maps
// directly to the CSS variable `--background`.
// `dark` is kept as an optional legacy alias so older saved payloads
// and callers don’t crash immediately.
// ──────────────────────────────────────────────────────────────
export type BrandColors = Omit<BrandColorsLegacy, "dark"> & {
  background: string;
...
export const transformVarsToGlobalStyleModel = (
  ...
  return {
    brand: {
      ...
      secondary: getVal("--secondary", "#2EA76A"),
      accent: getVal("--accent", "#B9F3D5"),
      background: getVal("--background", getVal("--dark", "#0B3A2A")),
      dark: getVal("--background", getVal("--dark", "#0B3A2A")),
      ring: getVal("--ring", "#2EA76A"),
      text: getVal("--text", "#0B2A1F"),
      mutedText: getVal("--muted-text", "#5E6E65"),
...
export const updateCssWithColors = (
  css: string,
  palette: {
    brand: Partial<BrandColors> & { dark?: string; background?: string };
    buttons: Record<string, ButtonColors>;
  },
): string => {
  let updatedCss = css;
  ...
  Object.entries(brandMap).forEach(([varName, key]) => {
    const value =
      (palette.brand as any)[key] ??
      (key === "background" ? (palette.brand as any).dark : undefined);
    if (value) {
      const regex = new RegExp(`${varName}\\s*:\\s*[^;]+`, "g");
      updatedCss = updatedCss.replace(regex, `${varName}: ${value}`);
    }
