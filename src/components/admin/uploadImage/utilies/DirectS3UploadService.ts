...
export class DirectS3UploadService {
  ...
  static async uploadFile(
    ...
    try {
      ...
      // Generate unique key
      const key = this.generateUniqueKey(file.name, userProfileId, projectId);
      ...
      if (
        ...
        try {
          ...
          const fallbackKey = this.generateUniqueKey(file.name, userProfileId, projectId);
  ...
  /**
   * Generate unique S3 key with folder structure using project ID or user profile ID
   */
  private static generateUniqueKey(
    ...
    // Prioritize project ID if provided
    if (projectId) {
      return `uploads/${projectId}/images/${nameWithoutExtension}_${timestamp}_${randomString}.${extension}`;
    }

    // Use user profile ID if provided
    if (userProfileId) {
      return `uploads/${userProfileId}/images/${nameWithoutExtension}_${timestamp}_${randomString}.${extension}`;
    }
