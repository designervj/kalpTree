...
// //     const final = [...products].filter((d) =>
// //       d.allcategories.includes(slug?._id),
// //     );

// //     const sorted = final.length > 0 ? final : [];
...
// //     // .flatMap((product: any) => product.attributes);
// //   }, [products]);
...
//   // Generate dynamic filters from variant attributes
...
//           // Check if any variant has the selected attribute values
...
//           <div className="text-right">
//             <div className="text-[13px] font-medium text-gray-500 mb-1">
//               Showing 1-{getSortedProducts().length} of{" "}
...
//               {/* Dynamic Filters from Variant Attributes */}
...
//                       <label
//                         key={valueIndex}
//                         className="flex items-center text-[13px] text-gray-700 cursor-pointer"
//                       >
//                         <input
//                           type="checkbox"
...
const ProductShowcase: React.FC<ProductShowcaseProps> = ({
  ...
  const ProductCard = ({ product }: { product: any }) => {
    ...
    return (
      <div
        ...
        style={{
          boxShadow:
            cardStyle === "elevated" ? "0 2px 8px rgba(0,0,0,0.1)" : "none",
          border: cardStyle === "bordered" ? "1px solid #e5e7eb" : "none",
        }}
      >
        <div
  ...
  const TopFilters = () => (
    <div className="flex flex-wrap gap-4 mb-8">
      {showCategoryFilter && (
        <select
          className="border px-4 py-2 text-sm font-semibold uppercase"
          style={{
            fontFamily,
            borderColor: primaryColor,
            color: primaryColor,
    ...
    <div className="min-h-screen bg-[#fdfdfd]" style={{ fontFamily }}>
      ...
      {showHeroSection && (
        ...
        >
          <div>
            <h1
              className={`text-${titleSize} uppercase drop-shadow-lg`}
              style={{
                color: titleColor,
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center py-5 border-b border-gray-200 mb-8 gap-5">
          <div className="flex gap-4">
            {filterPosition === "sidebar" && (
              <button
                onClick={toggleSidebar}
                className="bg-white border px-5 py-3 text-xs font-semibold uppercase transition-colors flex items-center gap-2.5"
                style={{
                  fontFamily,
                  borderColor: primaryColor,
                  color: primaryColor,
            ...
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value)}
              className="bg-white border px-5 py-3 pr-9 text-xs font-semibold uppercase appearance-none cursor-pointer"
              style={{
                fontFamily,
                borderColor: primaryColor,
                color: primaryColor,
