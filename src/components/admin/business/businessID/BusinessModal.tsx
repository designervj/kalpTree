...
function buildBusinessForm(b: any) {
  return {
    ...
    // ── website (nested in business object) ──
    website: {
      name: b?.website?.name || b.name || "",
      serviceType: b?.website?.serviceType || "WEBSITE",
      primaryDomain: b?.website?.primaryDomain || [],
      lang: b?.website?.lang || [],
      isComingSoon: b?.website?.isComingSoon ?? false,
    },
  };
}
...
function BusinessTypeSelector({
  ...
  const finalTypes = React.useMemo(
    () => types.filter((d) => d.categoryId == industryId),
    [types, industryId],
  );
...
function BusinessForm({
  ...
  const setSt = (k: string, v: any) =>
    set("settings", { ...fd.settings, [k]: v });
  const setWeb = (k: string, v: any) =>
    set("website", { ...fd.website, [k]: v });

  React.useEffect(() => {
    if (!fd.businessdetails.industry) return;
    (async () => {
      try {
        const req = await fetch("/api/admin/attributessets");
        const res = await req.json();
        setBusinessType(res.items?.length ? res.items : []);
      } catch (e) {
        toast.error(String(e));
      } finally {
        setBusinessTypeLoading(false);
  ...
  return (
    <div className="flex gap-6 h-full min-h-0">
      ...
      <div className="flex-1 overflow-y-auto pr-2 min-h-0">
        ...
        {/* ── Website (replaces separate website modal) ── */}
        {sec === "website" && (
          <>
            <SectionHeader
              icon={Globe}
              title="Website"
              subtitle="Name · Service type · Domains · Languages · Visibility"
            />
            <div className="space-y-6">
              ...
              <div>
                <label className={lbl}>Website Name</label>
                <input
                  value={fd.website.name}
                  onChange={(e) => setWeb("name", e.target.value)}
                  placeholder="My Website"
                  className={inp}
                />
              </div>
                ...
                <div className="grid grid-cols-2 gap-3 mt-1">
                  {[
                    ...
                    <button
                      key={String(val)}
                      type="button"
                      onClick={() => setWeb("isComingSoon", val)}
                      className={cn(
                        "flex flex-col items-start rounded-xl border px-4 py-3.5 text-left transition-all",
                        fd.website.isComingSoon === val
                          ? "border-slate-900 bg-slate-900 text-white"
                          : "border-gray-200 bg-white text-gray-700 hover:bg-gray-50",
                      )}
                    >
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3 mt-1">
                  {SERVICE_OPTIONS.map(({ value, title, desc, Icon }) => {
                    const sel = fd.website.serviceType === value;
                    return (
                      <button
                        key={value}
                        type="button"
                        onClick={() => setWeb("serviceType", value)}
                        className={cn(
                          "relative flex items-start gap-3 rounded-xl border p-4 text-left transition-all",
                          sel
                            ? "border-slate-900 bg-slate-900 ring-1 ring-slate-900"
              ...
              <div>
                ...
                <LanguageSelector
                  formData={fd.website}
                  handleInputChange={(e: any) => setWeb("lang", e.target.value)}
                />
              </div>

              {/* Domains */}
              <div>
                <label className={lbl}>Primary Domains</label>
                <div className="space-y-2 mt-1">
                      ...
                      <div key={i} className="flex items-center gap-2">
                        <div className="flex items-center flex-1 border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-slate-900/15 focus-within:border-slate-900 bg-white transition-all">
                          ...
                          <input
                            value={domain}
                            onChange={(e) => {
                              const updated = [...fd.website.primaryDomain];
                              updated[i] = e.target.value;
                              setWeb("primaryDomain", updated);
                            }}
                            placeholder="example.com"
                            className="flex-1 px-2 py-2.5 outline-none text-sm"
                          />
                        ...
                        <button
                          ...
                          onClick={() =>
                            setWeb(
                              "primaryDomain",
                              fd.website.primaryDomain.filter(
                                (_: any, idx: number) => idx !== i,
                              ),
                            )
                          }
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setWeb("primaryDomain", [
                        ...(fd.website.primaryDomain || []),
                        "",
                      ])
                    }
                    className="flex items-center gap-2 text-sm font-medium text-slate-900 hover:text-slate-700 transition-colors mt-1"
                  >
                    <div className="flex h-7 w-7 items-center justify-center rounded-md border border-gray-200 bg-white">
                      <Plus className="h-3.5 w-3.5" />
                    </div>
...
function buildPayload(fd: any) {
  return {
    ...
    features: fd.features,
    settings: fd.settings,
    website: fd.website, // nested website object saved together
  };
}

// ─── Main Modal ───────────────────────────────────────────────────────────────

export const BusinessModal = ({ open, business, onClose }: ModalProps) => {
  const [formData, setFormData] = React.useState<any>(null);
  const [isSaving, setIsSaving] = React.useState(false);
  const [error, setError] = React.useState<string | null>(null);
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await fetch(`/api/admin/business/${business._id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">
                Account · Branding · Details · Website · Features · Settings
              </p>
            </div>
          </div>
          <button
