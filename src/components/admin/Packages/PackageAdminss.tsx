...
const INIT_PACKAGES = [
  {
    ...
    additionalInfo: {
      aboutDestination:
        "Known as the 'Island of the Gods', Bali is renowned for its forested volcanic mountains, iconic rice paddies, beaches, and coral reefs. The island is home to religious sites such as cliffside Uluwatu Temple. To the south, the beachside town of Kuta is known for surfing, nightlife and shops.",
      quickInfo: {
    ...
    faqs: [
      {
        ...
        answer:
          "No, international airfare is not included. The package covers all local transfers, accommodation, and listed activities. We recommend booking flights to Ngurah Rai International Airport (DPS).",
      },
      {
        id: "faq-2",
        question: "What is the best time to visit Bali?",
        answer:
          "October to March is ideal — warm weather, clear skies, and festive energy. Our package runs during these peak months for the best experience.",
      },
      {
        id: "faq-3",
        question: "Is this package suitable for solo travelers?",
        answer:
          "Absolutely. You'll have your own private villa room and a dedicated guide throughout. Solo supplements may apply — please enquire at booking.",
      },
    ],
    itinerary: [
      {
...
const Ic = {
  ...
  Chevron: ({ open }) => (
    <svg
      className={cls("w-4 h-4 transition-transform", open && "rotate-180")}
      fill="none"
      stroke="currentColor"
      viewBox="0 0 24 24"
    >
  ...
  Car: () => (
    ...
    >
      ...
      <path
        ...
        strokeLinejoin="round"
        strokeWidth={2}
        d="M13 16H7m9.293-9H6l-3 6h18l-1-4.5H16.29"
      />
    </svg>
  ),
  Star: () => (
      ...
      <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-blue-700 mt-0.5">
            ...
            . Editing a master record auto-updates all linked packages via
            populate().
          </p>
        </div>
      </div>
      ...
      <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-emerald-700 mt-0.5">
            ...
            . Edit master → all packages reflect changes instantly via
            populate().
          </p>
        </div>
      </div>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between p-4 bg-blue-950 text-white rounded-xl">
        <div>
          <h2 className="text-sm font-bold">
            {pkg?.title || "Package"} — Summarised Itinerary
          </h2>
          <p className="text-xs text-blue-300 mt-0.5">
            {itinerary.length} days · read-only overview
          </p>
        </div>
        <div className="flex gap-2">
          <Btn
            ...
            onClick={() => setOpenDays(new Set(itinerary.map((d) => d.id)))}
          >
            Expand All
          </Btn>
          <Btn
      ...
      {itinerary.map((day) => {
        ...
        const rHotels = (day.hotelStays || []).map((h) =>
          resolveHotel(h, masterHotels),
        );
        const rActs = (day.activities || [])
          .map((a) => ({
            ...resolveActivity(a, masterActivities),
            time: a.time,
          }))
          .sort((a, b) => (a.time || "").localeCompare(b.time || ""));
        const hasItems =
          rHotels.length + (day.transfers?.length || 0) + rActs.length > 0;
        return (
          <div
            key={day.id}
          ...
          >
            <button
              ...
              className={cls(
                "w-full flex items-center gap-4 px-5 py-3.5 text-left bg-gradient-to-r text-white",
                DAY_GRAD[day.dayType] || "from-blue-950 to-blue-900",
              )}
            >
              <div className="w-8 h-8 rounded-lg bg-white/20 flex items-center justify-center text-sm font-bold flex-shrink-0">
                {day.dayNumber}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-4 mt-0.5 text-xs opacity-70">
                  {day.transfers?.length > 0 && (
                    <span className="flex items-center gap-1">
                      <Ic.Car />
                      {day.transfers.length} transfer
                      {day.transfers.length > 1 ? "s" : ""}
                    </span>
                  )}
                  {rActs.length > 0 && (
                    <span className="flex items-center gap-1">
                      <Ic.Activity />
                      {rActs.length} activit{rActs.length > 1 ? "ies" : "y"}
                    </span>
                  )}
                  {rHotels.length > 0 && (
                    <span className="flex items-center gap-1">
                      <Ic.Hotel />
                      {rHotels.length} hotel{rHotels.length > 1 ? "s" : ""}
                    </span>
                  )}
                  {day.mealsIncluded?.length > 0 && (
                    <span>🍽 {day.mealsIncluded.join(" + ")}</span>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white divide-y divide-gray-50">
                ...
                {day.mealsIncluded?.length > 0 && (
                  <div className="px-5 py-3 flex items-center gap-3">
                    <span className="text-xs font-semibold text-gray-500 w-20 flex-shrink-0">
                      Meals
                    </span>
                    <div className="flex gap-2">
                      {day.mealsIncluded.map((m) => (
                        <span
                          ...
                          className={cls(
                            ...
                            mealCls[m] || "bg-gray-100 text-gray-700",
                          )}
                        >
                          {m}
                        </span>
                  ...
                  <div className="px-5 py-3">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs font-bold text-gray-600 uppercase tracking-wider">
                        Transfers ({day.transfers.length})
                      </span>
                    </div>
                    <div className="space-y-2 ml-7">
                      {day.transfers.map((tr) => (
                        ...
                        >
                          <span className="font-semibold text-gray-400 w-14 flex-shrink-0">
                            {fmt12(tr.pickupTime)}
                          </span>
                          ...
                          <div className="flex items-center gap-1 text-orange-400 flex-shrink-0">
                            <div className="w-3 h-px bg-orange-200" />
                            <Ic.Arrow />
                            <div className="w-3 h-px bg-orange-200" />
                          </div>
                          <span className="font-semibold truncate max-w-[90px]">
                      ...
                      {rActs.map((act, i) => (
                        <div
                          key={
                            act.id ||
                            act.activityRef ||
                            `${act.title}-${act.time}-${i}`
                          }
                          className="flex items-start gap-3"
                        >
                          <span className="text-xs font-mono text-gray-400 w-14 flex-shrink-0 mt-0.5">
                            {fmt12(act.time)}
                          </span>
                          <div
                            className={cls(
                              "w-2 h-2 rounded-full mt-1.5 flex-shrink-0",
                              ACT_DOT[act.activityType] || "bg-gray-400",
                            )}
                          />
                          <div className="flex-1 min-w-0">
                            ...
                            {act.images?.length > 0 && (
                              <div className="mt-2">
                                {act.coverTitle && (
                                  <p className="text-xs font-semibold text-blue-900 mb-1.5 italic">
                                    "{act.coverTitle}"
                                  </p>
                                )}
                  ...
                  <div className="px-5 py-4">
                    ...
                    <div className="space-y-3 ml-7">
                      {rHotels.map((h) => (
                        <div
                          key={h.id}
                          className="rounded-xl border border-emerald-100 bg-emerald-50/30 overflow-hidden"
                        >
                          {/* Hotel image strip */}
                          {h.images?.length > 0 && (
                            <div
                            ...
                            >
                              ...
                              {h.images.length > 3 && (
                                <div className="absolute bottom-2 right-2 bg-black/60 text-white text-xs px-2 py-1 rounded-lg font-semibold">
                                  +{h.images.length - 3} more
                                </div>
                              )}
                            </div>
                          )}
                                  ...
                                  {[
                                    ...
                                    return (
                                      <div
                                        key={meal}
                                        className={cls(
                                          "flex flex-col items-center gap-1 py-2.5 rounded-xl border",
                                          inc
                ...
                {!hasItems && (
                  <div className="px-5 py-4 text-center text-xs text-gray-400 italic">
                    No activities, hotels or transfers added yet.
                  </div>
                )}
              </div>
            )}
      ...
      <div className="flex flex-wrap gap-3 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl">
        ...
        {[
          ...
          >
            <div className={cls("w-2.5 h-2.5 rounded-full", ACT_DOT[type])} />
            {label}
          </div>
        ))}
        <div className="flex items-center gap-1 text-xs text-emerald-600 ml-2 font-semibold">
          <Ic.Sync />
          =Master linked
        </div>
      </div>
...
const ItineraryBuilder = ({ itinerary, setItinerary }) => {
  ...
  const updateDay = (id, f, v) =>
    setItinerary((p) => p.map((d) => (d.id === id ? { ...d, [f]: v } : d)));
  const addHotel = (did) =>
    setItinerary((p) =>
      p.map((d) =>
        d.id === did
  ...
  const removeHotel = (did, hid) =>
    setItinerary((p) =>
      p.map((d) =>
        d.id === did
          ? { ...d, hotelStays: d.hotelStays.filter((h) => h.id !== hid) }
          : d,
      ),
    );
  const updateHotel = (did, hid, f, v) =>
    setItinerary((p) =>
      p.map((d) =>
        d.id === did
          ? {
  ...
  const removeAct = (did, aid) =>
    setItinerary((p) =>
      p.map((d) =>
        d.id === did
          ? { ...d, activities: d.activities.filter((a) => a.id !== aid) }
          : d,
      ),
    );
  const updateAct = (did, aid, f, v) =>
    setItinerary((p) =>
      p.map((d) =>
        d.id === did
          ? {
  ...
  const removeTr = (did, tid) =>
    setItinerary((p) =>
      p.map((d) =>
        d.id === did
          ? { ...d, transfers: d.transfers.filter((t) => t.id !== tid) }
          : d,
      ),
    );
  const updateTr = (did, tid, f, v) =>
    setItinerary((p) =>
      p.map((d) =>
        d.id === did
          ? {
                  ...
                  <div className="space-y-2">
                    {day.hotelStays.map((hs) => (
                      <HotelPicker
                        ...
                        dayHotel={hs}
                        dayId={day.id}
                        onUpdate={(did, hid, f, v) =>
                          updateHotel(did, hid, f, v)
                        }
                        onRemove={(did, hid) => removeHotel(did, hid)}
                      />
                    ))}
                  </div>
                </div>
...
const EditableListItem = ({
  ...
  if (
    ...
    document.activeElement?.dataset?.itemid !== String(value)
  ) {
    prevRef.current = value;
    setLocal(value);
  }
  return (
    ...
    >
      <span className={cls("flex-shrink-0 font-bold text-sm", iconCls)}>
        {icon}
      </span>
      <input
        type="text"
        ...
        onChange={(e) => setLocal(e.target.value)}
        onBlur={() => {
          if (local.trim() !== value) onBlur(local.trim() || value);
        }}
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            Add inclusions (what's covered) and exclusions (what's not covered)
          </p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div className="grid grid-cols-2 divide-x divide-gray-200">
          <div className="px-5 py-3 bg-emerald-50 border-b border-gray-200 flex items-center gap-2">
            <div className="w-5 h-5 rounded-full bg-emerald-600 text-white flex items-center justify-center text-xs font-bold flex-shrink-0">
              ✓
            </div>
        ...
        {(inclusions.length > 0 || exclusions.length > 0) && (
          <div className="divide-y divide-gray-100">
            {Array.from({
              length: Math.max(inclusions.length, exclusions.length),
            }).map((_, i) => (
              <div
                key={i}
                ...
                <div className="px-4 py-1">
                  {inclusions[i] !== undefined ? (
                    <EditableListItem
                      value={inclusions[i]}
                      onBlur={(v) =>
                        onChangeInc(inclusions.map((x, j) => (j === i ? v : x)))
                      }
                      onRemove={() =>
                        onChangeInc(inclusions.filter((_, j) => j !== i))
                      }
                      icon="✓"
                      rowCls="bg-white"
                ...
                <div className="px-4 py-1">
                  {exclusions[i] !== undefined ? (
                    <EditableListItem
                      value={exclusions[i]}
                      onBlur={(v) =>
                        onChangeExc(exclusions.map((x, j) => (j === i ? v : x)))
                      }
                      onRemove={() =>
                        onChangeExc(exclusions.filter((_, j) => j !== i))
                      }
                      icon="✕"
                      rowCls="bg-white"
        ...
        <div className="grid grid-cols-2 divide-x divide-gray-200 border-t border-gray-200 bg-gray-50/60">
          <div className="px-4 py-3 flex gap-2">
            ...
            <button
              type="button"
              onClick={addInc}
              className="px-3 py-1.5 text-xs font-bold bg-emerald-600 text-white rounded-lg hover:bg-emerald-500 transition-colors flex-shrink-0 flex items-center gap-1"
            >
              <Ic.Plus />
              Add
            </button>
          ...
          <div className="px-4 py-3 flex gap-2">
            ...
            <button
              type="button"
              onClick={addExc}
              className="px-3 py-1.5 text-xs font-bold bg-red-500 text-white rounded-lg hover:bg-red-400 transition-colors flex-shrink-0 flex items-center gap-1"
            >
              <Ic.Plus />
              Add
            </button>
...
const KBYGItem = ({ item, index, onBlur, onRemove }) => {
  ...
  if (prevRef.current !== item.point) {
    prevRef.current = item.point;
    setLocal(item.point);
  }
  return (
    <div className="flex items-start gap-3 p-3 bg-amber-50 border border-amber-200 rounded-xl group">
      ...
      <textarea
        ...
        onChange={(e) => setLocal(e.target.value)}
        onBlur={() => {
          if (local.trim() !== item.point) onBlur(local.trim() || item.point);
        }}
        rows={2}
      ...
      <button
        type="button"
        onClick={onRemove}
        className="flex-shrink-0 p-1 text-gray-400 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100 rounded-lg hover:bg-red-50 mt-0.5"
      >
        <Ic.Trash />
      </button>
    </div>
  ...
  return (
    <Card className="p-6">
      <div className="flex items-start gap-3 mb-5">
        <div className="w-7 h-7 rounded-lg bg-blue-950 text-white flex items-center justify-center text-sm font-bold flex-shrink-0">
          5
        </div>
        <div>
          <div className="flex items-center gap-2">
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            Important traveller guidelines, policies and notices
          </p>
        </div>
      </div>
      ...
      {points.length === 0 && (
        <div className="py-8 text-center border-2 border-dashed border-amber-200 rounded-xl bg-amber-50/30 mb-4">
          ...
          <p className="text-xs text-gray-400 mt-1">
            Add important policies travellers should know before booking
          </p>
        </div>
      )}
        ...
        <div className="space-y-2 mb-4">
          {points.map((pt, i) => (
            <KBYGItem
              ...
              onBlur={(v) =>
                onChange(
                  points.map((p) => (p.id === pt.id ? { ...p, point: v } : p)),
                )
              }
      ...
      <div className="flex gap-2">
        ...
        />
        <Btn variant="primary" size="sm" onClick={handleAdd}>
          <Ic.Plus />
          Add Point
        </Btn>
      </div>
  ...
  return (
    ...
    >
      ...
      <input
        ...
        onBlur={() => {
          if (loc !== value) onCommit(loc || value);
        }}
    ...
    <Card className="p-6">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            Destination overview, quick info, experiences and highlights
          </p>
        </div>
      </div>
      <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3 p-4 bg-blue-50 border border-blue-100 rounded-xl">
            {[
              ...
              [
                "endPoint",
                "🏁 End Point",
                "e.g. Bangkok International Airport (BKK)",
              ],
            ].map(([key, label, ph]) => (
              <div key={key}>
        ...
        <div>
          ...
          <div className="space-y-2 mb-2">
            ...
            {exps.map((exp, i) => (
              <StringListItem
                ...
                onCommit={(v) => {
                  const next = exps.map((x, j) => (j === i ? v : x));
                  setExps(next);
                  flush({ experiencesCovered: next });
                }}
                onRemove={() => {
                  const next = exps.filter((_, j) => j !== i);
                  setExps(next);
                  flush({ experiencesCovered: next });
                }}
              />
          ...
          <div className="flex gap-2">
            <input
              type="text"
              placeholder="e.g. Snorkeling at Coral Island…"
              value={newExp}
              ...
              className="flex-1 px-3 py-2 text-sm text-gray-900 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-900/20 bg-white placeholder:text-gray-400"
            />
            <Btn variant="primary" size="sm" onClick={addExp}>
              <Ic.Plus />
              Add
            </Btn>
          </div>
        ...
        <div>
          ...
          <div className="space-y-2 mb-2">
            ...
            {ntm.map((item, i) => (
              <StringListItem
                ...
                onCommit={(v) => {
                  const next = ntm.map((x, j) => (j === i ? v : x));
                  setNtm(next);
                  flush({ notToMiss: next });
                }}
                onRemove={() => {
                  const next = ntm.filter((_, j) => j !== i);
                  setNtm(next);
                  flush({ notToMiss: next });
                }}
              />
          ...
          <div className="flex gap-2">
            <input
              type="text"
              placeholder="e.g. Kecak fire dance at Uluwatu Temple…"
              value={newNtm}
              onChange={(e) => setNewNtm(e.target.value)}
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
                  addNtm();
              ...
              className="flex-1 px-3 py-2 text-sm text-gray-900 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-900/20 bg-white placeholder:text-gray-400"
            />
            <Btn variant="primary" size="sm" onClick={addNtm}>
              <Ic.Plus />
              Add
            </Btn>
          </div>
  ...
  return (
    <div className="border border-blue-100 rounded-xl overflow-hidden bg-white shadow-sm">
      <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-blue-50 to-slate-50 border-b border-blue-100">
        <div className="w-6 h-6 rounded-lg bg-blue-950 text-white flex items-center justify-center text-xs font-bold flex-shrink-0">
          {index + 1}
        </div>
        ...
        <div className="p-4 space-y-3">
          <div>
            ...
            <input
              ...
              onBlur={() => {
                if (q !== faq.question) onUpdate(faq.id, "question", q);
              }}
              className="w-full px-3 py-2 text-sm text-gray-900 border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-900/20 placeholder:text-gray-400 transition-all"
            />
          </div>
          <div>
            ...
            <textarea
              ...
              onBlur={() => {
                if (a !== faq.answer) onUpdate(faq.id, "answer", a);
              }}
              className="w-full px-3 py-2 text-sm text-gray-900 border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-900/20 placeholder:text-gray-400 transition-all resize-none"
            />
            {a && (
      ...
      <div className="flex items-start justify-between mb-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Common traveller questions — shown as an accordion on the package
              page
            </p>
          </div>
        </div>
        <Btn variant="primary" size="sm" onClick={addFaq}>
          <Ic.Plus />
          Add FAQ
        </Btn>
      </div>
      {faqs.length === 0 && (
        <div className="py-10 text-center border-2 border-dashed border-blue-200 rounded-xl bg-blue-50/40">
          ...
          <p className="text-xs text-gray-400 mt-1 mb-4">
            Add common questions travellers ask about this package
          </p>
          <Btn variant="soft" size="sm" onClick={addFaq}>
            <Ic.Plus />
            Add your first FAQ
          </Btn>
        </div>
        ...
        <div className="mt-4 text-center">
          ...
          >
            {showAll ? (
              ...
              <>
                ↓ Load {faqs.length - PREVIEW} more FAQ
                {faqs.length - PREVIEW > 1 ? "s" : ""}
              </>
            )}
          </button>
        </div>
...
const ViewPackage = ({ pkg, onEdit }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-blue-300 flex items-center gap-1 mt-1">
              <Ic.MapPin />
              {pkg.destination} · {pkg.tripDuration}
            </p>
            ...
            {pkg.price?.amount && Number(pkg.price.amount) > 0 ? (
              <>
                <p className="text-3xl font-bold">
                  {sym}
                  {Number(pkg.price.amount).toLocaleString("en-IN")}
                </p>
                <p className="text-xs text-blue-400 mt-1">
                  {pkg.price?.currency} / person
                </p>
              </>
            ) : (
      ...
      <div className="flex border-b border-gray-200 overflow-x-auto">
        {tabs.map(([key, label]) => (
          <button
            ...
            onClick={() => setTab(key)}
            className={cls(
              "flex-shrink-0 px-5 py-3 text-sm font-semibold border-b-2 transition-all whitespace-nowrap",
              tab === key
      ...
      {/* Summary Tab */}
      {tab === "summary" && (
        <SummarisedView itinerary={pkg.itinerary || []} pkg={pkg} />
      )}
      ...
      {tab === "additional" &&
        (() => {
          ...
          if (
            !ai ||
            (!ai.aboutDestination &&
              !ai.quickInfo?.destinationsCovered &&
              !ai.experiencesCovered?.length &&
              !ai.notToMiss?.length)
          )
            return (
              <div className="py-16 text-center border-2 border-dashed border-blue-200 rounded-xl bg-blue-50/40">
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Edit this package to add destination info
                </p>
              </div>
            );
          return (
            <div className="space-y-4">
              ...
              {/* Quick Info */}
              {(ai.quickInfo?.destinationsCovered ||
                ai.quickInfo?.duration ||
                ai.quickInfo?.startPoint ||
                ai.quickInfo?.endPoint) && (
                <Card className="p-6">
                  ...
                  <ul className="space-y-2.5">
                    {ai.experiencesCovered.map((exp, i) => (
                      <li
                        ...
                        className="flex items-start gap-3 text-sm text-gray-700"
                      >
                        <span className="text-violet-500 font-bold flex-shrink-0 mt-0.5">
                          ✦
                        </span>
                  ...
                  <ul className="space-y-2.5">
                    {ai.notToMiss.map((item, i) => (
                      <li
                        ...
                        className="flex items-start gap-3 text-sm text-gray-700"
                      >
                        <span className="text-rose-500 font-bold flex-shrink-0 mt-0.5">
                          ★
                        </span>
                        <span className="leading-snug">{item}</span>
                      </li>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <Btn className="ml-auto" onClick={() => setPage("create")}>
          <Ic.Plus />
          Create Package
        </Btn>
      </div>
      ...
      <div className="p-4 bg-gradient-to-r from-blue-950 to-blue-900 rounded-xl text-white flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-blue-300 mt-0.5">
            ...
            ObjectIds. Edit a master record → all packages reflect changes
            instantly via MongoDB populate().
          </p>
        </div>
...
const Sidebar = ({ page, setPage, counts }) => {
  ...
  const isActive = (key) =>
    key === "dashboard"
      ? page === "dashboard"
      : key === "packages"
        ? ["packages", "create", "edit", "view"].includes(page)
        : page === key;
  return (
    <aside className="fixed left-0 top-0 h-screen w-60 bg-blue-950 text-white flex flex-col z-30 shadow-xl">
      <div className="px-5 py-5 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-blue-400">Admin Panel v6</div>
          </div>
        </div>
      </div>
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        ...
        <p className="text-xs font-bold text-blue-500 uppercase tracking-widest px-3 pt-4 pb-2">
          Master Catalog
        </p>
        <div className="space-y-0.5">
          {nav
            .filter((n) => n.group === "master")
            .map((item) => (
              <button
                key={item.key}
                onClick={() => setPage(item.key)}
      ...
      <div className="px-4 py-3 mx-3 mb-4 bg-blue-900/50 rounded-xl border border-blue-800/60 text-xs text-blue-300 space-y-0.5">
        ...
        <p>✓ Master Activity catalog</p>
        <p>✓ Master Hotel catalog</p>
        <p>✓ Two-way activityRef/hotelRef sync</p>
        <p>✓ Summarised View tab</p>
        <p>✓ Override fields per package</p>
      </div>
      <div className="px-5 py-3 border-t border-white/10">
        <p className="text-xs text-blue-600">© 2025 Aventara Elite</p>
      </div>
    </aside>
  );
};
...
export default function PackagePanel() {
  ...
  const PAGE_META = {
    ...
    "master-activities": {
      ...
      subtitle: `${masterActivities.length} reusable activities in global catalog`,
    },
    "master-hotels": {
      title: "Master Hotels",
      ...
      <div className="min-h-screen bg-gray-50/80">
        ...
        <div className="p-6 max-w-[1400px]">
          {["create", "edit", "view"].includes(page) && (
            <button
              onClick={() => setPage("packages")}
              className="inline-flex items-center gap-1.5 text-sm text-blue-900 font-semibold mb-5 hover:underline"
            >
              <Ic.Back />
              Back to Packages
            </button>
          ...
          {page === "packages" && (
            <PackagesListing setPage={setPage} setSelectedId={setSelectedId} />
          )}
          {page === "create" && (
            <PackageForm
              initial={emptyPkg()}
