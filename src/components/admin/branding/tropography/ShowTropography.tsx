...
import {
  GlobalStyleModel,
  transformRawToGlobalStyleModel,
} from "@/components/editor/style-editor/GlobalStyelModel";
import {
  BodyStyle,
  BrandColors,
  BtnKey,
  ...
  LeftTab,
  RightPanelTab,
} from "../../settings/global-styles/GlobalStyleModal";
...
import {
  ...
  rgba,
  shadowToCss,
} from "../../settings/global-styles/util/ColorFunction";
...
const ShowTropography = () => {
  ...
  const [currentStyle, setCurrentStyle] =
    React.useState<GlobalStyleModel | null>(null);
  ...
  const [buttonColors, setButtonColors] =
    useState<Record<BtnKey, ButtonColors>>();
  ...
  useEffect(() => {
    if (currentBusiness && currentBusiness.website?.globalStyle) {
      const updatedStyle = transformRawToGlobalStyleModel(
  ...
  const uiPalette = useMemo(() => {
    ...
    const dark = {
      ...
      mutedText:
        currentStyle?.themes?.dark?.mutedText ||
        mixHex("#EAF7F0", "#000000", 0.35),
      border:
        currentStyle?.themes?.dark?.border ||
        rgba(brand?.accent ?? "#B9F3D5", 0.22),
    };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const copied = structuredClone(currentBusiness);
      if (copied && copied.website && copied.website.globalStyle) {
        copied.website.globalStyle = data;
      }

      const response = await dispatch(
        updateBusiness({
          businessId: currentBusiness._id.toString(),
          input: copied,
        }),
      ).unwrap();
    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          {leftTab === "buttons" && buttonBase && buttonColors && (
            <ButtonControl
