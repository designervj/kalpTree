...
const AddHeader = ({ header, isEdit }: Props) => {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!currentBusiness?._id || !currentBusiness?.tenantId) {
    ...
    try {
      ...
      if (
        createHeader.fulfilled.match(result) ||
        updateHeader.fulfilled.match(result)
      ) {
        ...
        toast.error(
          (result.payload as string) || `Failed to ${actionText} header`,
        );
