...
import InventoryModal from "@/components/admin/booking/InventortAddModal";
...
const generateDailyData = (
  ...
  dates.forEach((d, index) => {
    const isWeekend =
      d.displayDate.includes("Sat") || d.displayDate.includes("Sun");
...
const GridCell: React.FC<{
  ...
  if (type === "master") {
    return (
      <div
        className={`border-r border-b border-gray-100 h-20 p-2 flex flex-col items-center justify-between min-w-[100px] ${isEven ? "bg-gray-50/30" : "bg-white"}`}
      >
  ...
  return (
    <div
      className={`border-r border-b border-gray-100 h-14 p-2 flex items-center justify-center min-w-[100px] ${isEven ? "bg-gray-50/30" : "bg-white"}`}
    >
      <input
        type="text"
...
const RoomRow: React.FC<{ room: RoomType; dates: CalendarDate[] }> = ({
  room,
  dates,
}) => {
          ...
          <React.Fragment key={plan.id}>
            ...
            <div className="contents">
              <div className="sticky left-0 z-10 bg-white border-b border-r border-gray-200 h-10 flex items-center justify-between px-4 pl-12 min-w-[250px] shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)] bg-gray-50/50">
                ...
                <button className="text-blue-500 hover:text-blue-600 text-xs font-medium hover:underline">
                  Edit
                </button>
              </div>
              {dates.map((_, idx) => (
                <div
                  key={idx}
                  className={`border-b border-r border-gray-100 h-10 ${idx % 2 !== 0 ? "bg-gray-50/30" : "bg-white"}`}
                />
              ))}
            </div>
...
export default function HotelInventoryCalendar() {
  ...
  useEffect(() => {
    (async () => {
      const req = await fetch(
        `/api/admin/bookings/calender?websiteId=696729dde1222ee82bdd906d&startDate=${startDate}&endDate=${endDate}`,
      );
  ...
  const handleChangeDate = (e: any) => {
    ...
    const formatedDate = formatToYYYYMMDD(new Date(value));
    if (name == "startDate") {
      setStartDate(formatedDate);
      setEndDate(formatToYYYYMMDD(addDays(new Date(value), 15)));
    } else if (name == "endDate") {
      const startdate = new Date(startDate);
      const enddate = new Date(value);
      if (enddate < startdate) {
        toast.error("Please Add Date Higher Than StartDate");
      } else {
  ...
  const handlePrevandNext15Days = (type: string) => {
    if (type == "next") {
      const newStartDate = addDays(new Date(endDate), 1);
      setStartDate(formatToYYYYMMDD(newStartDate));
      setEndDate(formatToYYYYMMDD(addDays(new Date(endDate), 16)));
    } else {
      const newStartDate = addDays(new Date(startDate), -16);
      setStartDate(formatToYYYYMMDD(newStartDate));
      setEndDate(formatToYYYYMMDD(addDays(new Date(newStartDate), 15)));
    }
  };

  const dates = generateDates(new Date(startDate), 15);
      ...
      <div className="max-w-5xl mx-auto">
        <header className="mb-6">
          ...
          <p className="text-gray-500 text-sm">
            Manage your room availability, pricing, and restrictions across all
            channels.
          </p>
        </header>
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto justify-between lg:justify-end">
            <div className="flex items-center gap-3">
              <div className="relative">
                <Button
                  variant="outline"
                  className="pr-8 border-gray-200 text-gray-600 font-medium h-9 shadow-sm"
                >
                  Export Actions
                </Button>
              ...
              <div className="relative">
                {/* <Button className="border-gray-200 text-white font-medium h-9 shadow-sm">
                  Add Inventory
                </Button> */}
                <InventoryModal />
              </div>
              ...
              <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden bg-white shadow-sm">
                <button
                  onClick={() => handlePrevandNext15Days("prev")}
                  className="px-3 py-2 hover:bg-gray-50 text-gray-600 border-r border-gray-200 transition-colors flex items-center gap-1 text-xs font-medium"
                >
                  <ArrowLeft size={14} /> Prev 15 Days
                </button>
                <button
                  onClick={() => handlePrevandNext15Days("next")}
                  className="px-3 py-2 hover:bg-gray-50 text-gray-600 transition-colors flex items-center gap-1 text-xs font-medium"
                >
        ...
        <div className="mt-6 flex flex-wrap items-center justify-between gap-4 text-xs text-gray-500 bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
          <div className="flex gap-6">
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 rounded-full bg-green-50 border border-green-100 flex items-center justify-center text-green-500">
                <Unlock size={10} />
              </div>
              <span>Open for Booking</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 rounded-full bg-red-50 border border-red-100 flex items-center justify-center text-red-500">
                <Lock size={10} />
              </div>
          ...
          <div>Last updated: Today at 09:42 AM</div>
        </div>
      </div>
