...
type HeadingStyle = {
  scale: number; // baseSize * scale => px
  weight: number;
  lineHeight: number;
...
export default function TypographyPage() {
      ...
      <Card>
        <CardContent className="pt-6 space-y-5">
          ...
          <ColorRow
            ...
            value={activeButton.hoverBg}
            onChange={(v) => setBtn({ hoverBg: v })}
          />
          <ColorRow
            label="Hover Text Color"
            value={activeButton.hoverText}
            onChange={(v) => setBtn({ hoverText: v })}
          />
          <ColorRow
            label="Hover Border Color"
            value={activeButton.hoverBorder}
            onChange={(v) => setBtn({ hoverBorder: v })}
          />
        </CardContent>
      </Card>
    );
  ...
  // helper for preview button computed styles
  const getBtnVisual = (k: BtnKey) => {
    ...
    return {
      ...
      className: `inline-flex items-center justify-center select-none outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 ring-offset-background ${shadowToClass(
        s.shadow
      )}`,
    };
  };
            ...
            <CardContent className="pt-6 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Base size affects heading px via scale. Body has its own size slider.
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="h-full min-h-[580px] border-2 border-muted/40 bg-white/50">
            ...
            <CardContent className="p-10 space-y-8" style={{ fontFamily }}>
              <div className={previewWidthClass}>
                ...
                {/* Buttons Preview (custom button to support hover styles) */}
                ...
                <p className="mt-3 text-xs text-muted-foreground">
                  Hover buttons to see hover colors/border. (We use custom{" "}
                  <span className="font-mono">&lt;button&gt;</span> for preview so hover styles work perfectly.)
                </p>
              </div>
            </CardContent>
          </Card>
