                  ...
                  <tbody className="divide-y divide-gray-100">
                     {orders.map((order) => (
                        <tr key={order.id} className="bg-white hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => setSelectedOrder(order)}>
                           ...
                           <td className="px-6 py-4">
                              <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium ${order.payment === 'Paid' ? 'bg-emerald-50 text-emerald-700' :
                                    order.payment === 'Refunded' ? 'bg-gray-100 text-gray-600' : 'bg-amber-50 text-amber-700'
                                 }`}>
                                 <div className={`w-1.5 h-1.5 rounded-full ${order.payment === 'Paid' ? 'bg-emerald-500' :
