...
export async function DELETE(req: Request) {
  try {
    ...
    const business = await tenantcoll.findOne({
    ...
    if (user.role === "agency" && business.tenantId?.toString() !== user.id) {
      return NextResponse.json(
        {
          error: "Forbidden: You don't have permission to delete this business",
        },
        { status: 403 },
      );
    }
    ...
    // Delete the business
    const result = await tenantcoll.deleteOne({
      _id: new ObjectId(businessId),
    });
    ...
    await pagesColl.deleteMany({ tenantId: new ObjectId(businessId) });
...
export async function PUT(req: Request) {
  try {
    ...
    // Update the business
    const result = await tenantcoll.updateOne(
      { _id: new ObjectId(businessId) },
      { $set: input },
    );
