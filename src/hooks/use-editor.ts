...
export function useEditor(containerId: string) {
  ...
  const cleanupComponentStylesAndScripts = (editor: GrapesJSEditor, component: any) => {
    ...
    try {
      ...
      console.log('✅ Cleaned up styles and scripts for deleted component:', {
        id: componentId,
        type: componentType,
        tagName: tagName,
        stylesRemoved: componentStyles.length,
        scriptsRemoved: componentScripts.length,
      });
    } catch (error) {
  ...
  const setupEventListeners = (editor: GrapesJSEditor) => {
    ...
    editor.on("component:remove", (component: any) => {
      ...
      cleanupComponentStylesAndScripts(editor, component);
      updateLayers(editor);
    });
  };
